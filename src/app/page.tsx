...
export default function Home() {
  ...
  function handleSendClick() {
    ...
    fetch('https://www.takenote-ai.bio/upload-audio/' + recordID, {
      ...
      .then((res: { script: string, answers: Array<{ key: string, value: string }> }) => {
        ...
        if (res) {
          if (res.answers)
            setResponseData((prev) => [...prev, ...res.answers])

          if (res.script)
            setScriptText((prev) => prev + ' ' + res.script)
        }
      })
      .catch(error => {
        console.error(error)
  ...
  return (
    ...
    <div>
      ...
      <section className='bg-white dark:bg-gray-900 h-full min-w-full'>
        <div
          className='py-8 px-4 mx-auto min-h-full max-w-screen-xl text-center lg:py-16 lg:px-12 space-y-10'>
          <a href='#'
             className='inline-flex justify-between items-center py-1 px-1 pr-4 mb-7 text-sm text-gray-700 bg-gray-100 rounded-full dark:bg-gray-800 dark:text-white hover:bg-gray-200 dark:hover:bg-gray-700'
             role='alert'>
            <span className='text-xs bg-primary-600 rounded-full text-white px-4 py-1.5 mr-3'>Lo nuevo</span> <span
            className='text-sm font-medium'>¡TakeNoteAI Cruz Roja está disponibe!</span>
          ...
          ¡Bienvenido/a a TakeNoteAI, nuestra plataforma líder en transcripción de voz a texto! Gracias a nuestros avanzados servicios de inteligencia artificial, podemos ayudarte a transformar tus grabaciones de audio en texto escrito de manera rápida y precisa.</p>
          <div className='mb-8 lg:mb-16 space-y-4 sm:flex-row sm:justify-center sm:space-y-0 sm:space-x-4'>

            <div
              className='p-5 flex flex-col border rounded-lg border-gray-500'>

              <div
                className='items-center'>

                {srcAudio ? (
                  <audio className='inline' controls src={srcAudio} />
                ) : (
                  <p></p>
                )}
                ...
                {!recording ? (
                  <button
                    className='m-4 relative inline-flex items-center justify-center p-0.5 mb-2 mr-2 overflow-hidden
                        text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-cyan-500
                    ...
                    onClick={() => createRecordStartRecording()}>
                      <span
              ...
              <div className='grid grid-cols-1 space-y-5 text-left'>

                <div
                  className='items-start'>
                  <p className='text-while font-bold'>{scriptText}</p>
                </div>

                <div className='space-y-5'>
                  {
                    responseData.map(it => (
                      <div className='items-start space-y-2' key={it.key}>
                        <span
      ...
      <section className='bg-white dark:bg-gray-900'>
        <div
          ...
          <img className='w-full hidden dark:block rounded rounded-lg'
               src='images/takenote-arch.png'
               alt='dashboard image' />
          <div className='mt-4 md:mt-0'>
            <h2 className='mb-4 text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white'>AWS Workflow</h2>
            <p className='mb-6 font-light text-gray-500 md:text-lg dark:text-gray-400'>
              TakeNoteAI es una aplicación de toma de notas basada en la nube, diseñada para capturar y almacenar
              información de los usuarios de manera segura y eficiente. La plataforma utiliza todo el poder y la
              escalabilidad de AWS para desplegar el proyecto, garantizando una experiencia de alta calidad para el
              usuario final.
            </p>
          </div>
        </div>
